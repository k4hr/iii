...
import type {EngineeringMetric, EngineeringModel, EngineeringSeverity} from '@/lib/engineering/build-engineering-model';
...
export type EngineeringVisualLabLabels = {
  ...
  metrics: Record<EngineeringMetric['key'], string>;
};

export function EngineeringVisualLab({model, labels}: {model: EngineeringModel; labels: EngineeringVisualLabLabels}) {
  ...
  const linked = useMemo(() => ({
    conditions: model.conditions.filter(item => selectedModule?.conditionIds.includes(item.id)),
    calculations: model.calculations.filter(item => selectedModule?.calculationIds.includes(item.id)),
    sources: model.sources.filter(item => selectedModule?.sourceIds.includes(item.id)),
    breakthroughs: model.breakthroughs.filter(item => selectedModule?.breakthroughIds.includes(item.id)),
    ...
    <section className="overflow-hidden rounded-[1.75rem] border border-cyan-100/[0.1] bg-[#020708]/95 shadow-[0_0_80px_rgba(24,215,203,.055)]">
      <header className="grid gap-5 border-b border-cyan-100/[0.08] bg-[linear-gradient(90deg,rgba(8,39,42,.72),rgba(0,0,0,.25))] px-5 py-6 sm:px-7 lg:grid-cols-[1fr_auto] lg:items-end">
        <div>
          ...
          <h2 className="mt-3 text-2xl font-semibold tracking-[-.035em] text-cyan-50 sm:text-3xl">{labels.title}</h2>
          <p className="mt-3 max-w-3xl text-xs leading-6 text-[#83a2a4]">{labels.description}</p>
        </div>
      ...
      <div className="grid gap-4 p-3 sm:p-5 xl:grid-cols-[minmax(0,1.5fr)_360px]">
        <div className="relative">
          ...
          />
          <EngineeringBlueprintOverlay modules={model.modules} onSelectModule={setSelectedModuleId} selectedModuleId={selectedModuleId} />
        ...
        <aside className="space-y-4">
          <div className="rounded-2xl border border-cyan-100/[0.08] bg-[linear-gradient(145deg,rgba(10,36,39,.72),rgba(0,0,0,.38))] p-5" ref={panelRef}>
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="mono-label">{labels.selectedModule}</div>
                <h3 className="mt-3 text-lg font-semibold text-cyan-50">{selectedModule.label}</h3>
              </div>
              <StatusPill label={labels.severity[selectedModule.severity]} severity={selectedModule.severity} />
            </div>
            <p className="mt-3 text-xs leading-6 text-[#7e9fa1]">{selectedModule.description}</p>
            <div className="mt-5">
              <div className="flex justify-between font-mono text-[9px] tracking-[.08em] text-cyan-100/45 uppercase"><span>{labels.moduleProgress}</span><span>{selectedModule.progress}%</span></div>
              <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-white/[0.06]"><div className="h-full rounded-full bg-gradient-to-r from-cyan-700 to-cyan-300" style={{width: `${selectedModule.progress}%`}} /></div>
            </div>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <MetricCard label={labels.artifactType} value={model.artifactLabel} />
            <MetricCard label={labels.gapOrders} value={model.gapOrders === null ? '—' : `${model.gapOrders} OOM`} />
            <MetricCard label={labels.criticalModules} value={String(model.criticalCount).padStart(2, '0')} />
            <MetricCard label={labels.sourceCandidates} value={String(model.sources.length).padStart(2, '0')} />
          </div>

          <LinkedPanel items={linked.conditions.map(item => ({id: item.id, label: item.title, href: item.href}))} label={labels.linkedBlockers} noItems={labels.noLinkedItems} open={labels.open} />
          <LinkedPanel items={linked.calculations.map(item => ({id: item.id, label: item.gapOrders === null || item.gapOrders === undefined ? item.title : `${item.title} / ${item.gapOrders} OOM`, href: item.href}))} label={labels.linkedCalculations} noItems={labels.noLinkedItems} open={labels.open} />
          <LinkedPanel items={linked.sources.map(item => ({id: item.id, label: item.title, href: item.href}))} label={labels.sourceCandidates} noItems={labels.noLinkedItems} open={labels.open} />
          {linked.breakthroughs.length > 0 && <LinkedPanel items={linked.breakthroughs.map(item => ({id: item.id, label: item.title, href: item.href}))} label={labels.activeBreakthroughs} noItems={labels.noLinkedItems} open={labels.open} />}
        </aside>
      </div>

      <div className="border-t border-cyan-100/[0.07] p-4 sm:p-5">
        <div className="mb-3 mono-label">{labels.charts}</div>
        <EngineeringChartsPanel labels={labels.metrics} metrics={model.metrics} />
      </div>
    </section>
  );
}
...
function LinkedPanel({label, items, noItems, open}: {label: string; items: Array<{id: string; label: string; href?: string}>; noItems: string; open: string}) {
  return (
      ...
      <div className="mt-3 space-y-2">
        {items.length ? items.slice(0, 4).map(item => (
          <div className="flex items-start justify-between gap-3 rounded-lg border border-cyan-100/[0.06] bg-white/[0.015] p-2.5" key={item.id}>
            <span className="text-[10px] leading-4 text-cyan-50/65">{item.label}</span>
