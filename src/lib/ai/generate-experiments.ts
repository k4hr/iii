...
export function generateExperiments(text: string) {
  const lower = text.toLowerCase();
  if (/time|врем|field|поле/.test(lower)) return [
  ...
  if (/battery|лит|lithium|батар/.test(lower)) return [
