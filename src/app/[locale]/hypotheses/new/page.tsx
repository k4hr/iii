...
export default async function NewHypothesis({params, searchParams}: {params: Promise<{locale:string}>; searchParams: Promise<{project?:string}>}) {
  const {locale}=await params; const sp=await searchParams; const t=await getTranslations('hypothesis'); const c=await getTranslations('common');
  ...
  return <div className="mx-auto max-w-3xl"><h1 className="mb-6 text-4xl font-black">{t('createTitle')}</h1><form action={createHypothesisAction.bind(null, locale)} className="lab-card space-y-4 p-6">
    <label className="block"><span className="mb-2 block text-sm text-slate-300">{c('project')}</span><select className="lab-input" name="projectId" defaultValue={sp.project || ''}><option value="">No project</option>{projects.map(p=><option key={p.id} value={p.id}>{p.title}</option>)}</select></label>
